        ...
        <div class="box box-warning">
          ...
          <form method="post" action="{{ route('item.update', ['id' => $item->id]) }}">
							...
							<div class="row">
								<div class="col-md-5">
									...
									<div class="form-group {{ $errors->has('model') ? 'has-danger' : '' }}">
										<label>Model</label>
										<input class="form-control" type="text" name="model" value="{{ $item->model }}" placeholder="Model" autofocus>
										@if ($errors->has('model'))
									...
									<!-- BRAND -->
									<!-- ----- -->
									<div class="form-group {{ $errors->has('brand_select') ? 'has-danger' : ($errors->has('brand_input') ? 'has-danger' : ($errors->has('brand_duplicate') ? 'has-danger' : '')) }}">
										<label>Brand</label>
										<span id="brand_select">
											...
											<select class="form-control select2" name="brand_select">
												@foreach ($brands as $brand)
													<option {{ $brand->id === $item->brand->id ? 'selected' : '' }} value="{{ $brand->id }}">{{ $brand->name }}</option>
												@endforeach
											</select>
									...
									<!-- CATEGORY -->
									<!-- -------- -->
									<div class="form-group {{ $errors->has('category_select') ? 'has-danger' : ($errors->has('category_input') ? 'has-danger' : ($errors->has('category_duplicate') ? 'has-danger' : '')) }}">
						...
						<div class="box-footer">
							<div class="row">
								<div class="col-md-5">
									<button value=0 name="updatebtn" type="submit" class="btn btn-primary">Update</button>
									<button value=1 name="updatebtn" type="submit" class="btn btn-info"><i class="fa fa-ticket"></i></button>
