...
@section('title', 'Edit Filed Overtime')
    ...
    <div class="row mt-4 mb-4">
      <div class="col-md-12">
        <div class="box box-warning">
          ...
          <form role="form" action="{{ route('overtime.update', ['id' => $overtime->id]) }}" method="post">
						{{ csrf_field() }}
					  <div class="box-body">
					  	<div class="row">
					  		<div class="col-md-8">
							  	...
							  	@if ($overtime->status === 2)
										<p>
											<strong>Returned By: </strong> {{ $overtime->remarks_user->first_name }} {{ $overtime->remarks_user->last_name }}
											<br>
											<strong>Remarks: </strong> {{ $overtime->remarks }}
										</p>
										<hr>
									@endif
									...
									<div class="row">
								        ...
								        <input type="text"
								          		...
								          		id="date_from"
								          		name="date_from"
								          		value="{{ \Carbon\Carbon::parse($overtime->date_from)->format('m/d/Y h:i a') }}">
								        ...
								        <input type="text"
								          		class="form-control"
								          		{{ $overtime->date_to === null ? 'disabled' : '' }}
								          		id="date_to"
								          		name="date_to"
								            		value="{{ \Carbon\Carbon::parse($overtime->date_to)->format('h:i a') }}">
								        ...
								        @if ($errors->has('date_to'))
								        	<p class="help-block">
												...
												<div id="onwards_wdo" class="overtime_checkbox">
													<label class="onwards"><input type="checkbox" name="onwards" {{ $overtime->date_to === null ? 'checked' : '' }}>&nbsp;Onwards</label>&nbsp;&nbsp;&nbsp;
													<label class="wdo"><input type="checkbox" name="working_dayoff" {{ $overtime->working_dayoff ? 'checked' : '' }}>&nbsp;Working Day Off</label>
												</div>
								      </div>
										</div>
									</div>
					  ...
					  <div class="box-footer">
					  	<div class="row">
						      ...
						      <a href="{{ route('overtimes') }}" type="submit" class="btn btn-default pull-right">Cancel</a>
						    </div>
						  </div>
					  </div>
					</form>
...
$(document).ready(function(){
	...
	$('#onwards_wdo .onwards').click(function () {
		var onwards = $("#onwards_wdo input").is(':checked');
		date_to(onwards);
	});
