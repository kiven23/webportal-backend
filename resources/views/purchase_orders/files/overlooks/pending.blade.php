...
@section('title', 'Pending Purchase Order Files')
        ...
        <div class="box">
          ...
          <div class="box-body table-responsive">
            ...
            <table
              ...
              id="file-table"
              data-operators='[{"url":"{{route('po.file.approval.approve',['id'=>':id'])}}","text":"Approve","icon":"fa fa-check-circle-o"},{"url":"{{route('po.file.approval.reject',['id'=>':id'])}}","text":"Reject","icon":"fa fa-times-circle"}]'
            >
              <thead>
                <tr>
                  <th>ID</th>
              ...
              <tbody>
                @foreach ($files as $index => $file)
                  <tr>
                    ...
                    <td>{{ $file->from_user }}</td>
                    <td>
                      {{ $file->to_user ? $file->to_user : '' }}
                      {{ $file->to_company ? $file->to_company : '' }}
                    </td>
                    <td>{{ $file->to_user ? "user-" . $file->to_user_id : "company-" . $file->to_company_id }}</td>
                    <td>
											@if ($file->status !== 3)
