...
@section('title', 'Readd Pending from Breakdown')
  ...
  <section class="content-header">
    ...
    <ol class="breadcrumb">
      ...
      @if (\Auth::user()->branch->id != $branch_id)
			...
			<li><a href="{{ route('pending.branch.breakdown', ['id' => $branch_id, 'date' => $date]) }}">Breakdown</a></li>
      ...
      <div class="col-md-12">
        <div class="box box-primary">
          <div class="box-header with-border">
            <h3 class="box-title">
              Re-Add Pending from a Breakdown
            </h3>
          </div>
          <div class="box-body">
          	@include ('errors.list')

						<form class="form" action="{{ route('pending.store_breakdown', ['id' => $pending->branch_id]) }}" method="post">
							{{ csrf_field() }}
							<input type="hidden" value="{{ $pending->created_at->toDateString() }}" name="created_at">
							<input type="hidden" value="{{ $branch_id }}" name="branch">

							<div class="row">
								<div class="col-md-3">
									<div class="form-group {{ $errors->has('docdate') ? 'has-error' : '' }}">
										<label class="control-label" for="docdate">Document Date</label>
										<input readonly="true" type="text" id="document-date" name="docdate" class="form-control datepicker" data-date-format="yyyy-mm-dd" value="{{ $pending->docdate->toDateString() }}" >
