...
class ColumnsSort extends Process
{
    ...
    {
        $this->nameSort = Util::get($this->params, '{name}', null);
        $this->labelSort = Util::get($this->params, '{label}', null);
        $this->fixedColumns = Util::get($this->params, 'fixedColumns', []);
    }
    ...
    protected function onInput($row)
    {
        ...
        foreach ($this->sortedColumns as $column) {
            $newRow[$column] = Util::get($row, $column, null);
        }
