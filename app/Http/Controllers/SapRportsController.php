...
class SapRportsController extends Controller
{
    public function incoming_crb_generate(request $req){
        ...
        // return  $table = \DB::connection('sqlsrv')->select(DB::raw("select SeriesName, Series from NNM1 WHERE SeriesName LIKE '%SANJ%'"));
        // function remap($segment){
        //    $query = DB::table('branches')->where('sap_segment', $segment)->pluck('sapcode')->first();
        //    $ex = explode('-', $query);
        //    return $ex[0];
        // }
        // return remap('002');
